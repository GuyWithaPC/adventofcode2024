...
fn file_grid_size(data: &str) -> (usize, usize) {
    (data.lines().map(|_| 1).sum(), data.lines().fold(0, |acc, x| x.len()))
}

fn file_to_grid(data: &str) -> HashMap<(usize, usize), char> {
    data.lines().scan(0, |row_no, row| {
        *row_no += 1;
...
fn follow_xmas(grid: &HashMap<(usize,usize),char>, loc: (isize, isize), dir: (isize, isize), last_char: char) -> usize {
...
fn part_1(data: &str) {
    ...
    for y in 0..rows {
        for x in 0..cols {
            if grid[&(y,x)] == 'X' {
                count += follow_xmas(&grid, (y.try_into().unwrap(),x.try_into().unwrap()), (0,0), 'X');
            }
        }
    }
